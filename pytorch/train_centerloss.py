...
parser.add_argument('--center_weight', type=str, help='center loss weight', default=6)
# general parameters
...
parser.add_argument('--checkpoint', type=str, help='checkpoint prefix', default="centerloss")
...
class CenterLoss(nn.Module):
  ...
  def forward(self, x, target):
    ...
    # center loss
    if True:
      centers = torch.index_select(normed_weight, dim=0, index=target).detach() # bs x feat_dim
    else:
...
class CenterLossModel(nn.Module):
  def __init__(self, dim=512, num_class=10572, norm_data=True, radius=10):
    ...
    self.num_class = num_class
    self.base = Resnet20()
    self.fc6 = CenterLoss(dim, num_class, norm_data=norm_data, radius=radius)
  def forward(self, x, target=None):
    feature = self.base(x) # batch_size x 512
    if self.training:
      prob, center_loss = self.fc6(feature, target)
      # the returned feature with be used to calculate center-loss
      return prob, feature, center_loss
    else:
      return feature

# model and optimizer
print("Loading model...")
model = CenterLossModel(num_class=args.num_class, norm_data=True, radius=args.radius)
print("Done!")
...
def train_epoch(train_loader, model, optimizer, epoch):
  ...
  for batch_idx, (data, label) in enumerate(train_loader):
    ...
    prob, feature, center_loss = model(data, label)
    if args.parallel:
    ...
    if batch_idx % args.print_freq == 0:
      print("Epoch %d/%d Batch %d/%d, (sec/batch: %.2fsec): loss_cls=%.3f (* 1), loss-center=%.5f (* %.4f), acc1=%.3f, lr=%.3f" % \
      (epoch, args.maxepoch, batch_idx, len(train_loader), batch_time.val, loss_cls.val,
      loss_center.val, center_weight, top1.val, scheduler.get_lr()[0]))
      if args.l2filter:
